# 第一阶段：Python 基础与AI概念引入

作为从 前端全栈工程师 转型的开发者，你已经具备了一定的编程思维和经验。这一阶段的目标是快速掌握 Python 的核心语法、现代开发实践，以及引入AI相关概念，为后续学习打下坚实基础。
...
6. **数据处理与AI基础** (为AI/RAG做准备)
   - **Pandas 库基础** (DataFrame, Series) - 类比前端数据处理
   - 数据读取与写入 (CSV, JSON, Excel)
   - 数据清洗与预处理
   - NumPy 基础 (数组操作)
   - **简单数据分析** - 为AI概念做铺垫
   - **Jupyter Notebook 使用** - AI开发标准工具
...
完成以下小项目来检验学习成果（每个项目都有明确的输出，提供正反馈）：

1. **基础计算器程序** - 实现基本的加减乘除运算，使用类型提示 (第1周目标)
2. **待办事项列表** - 使用列表存储任务，实现添加、删除、查看功能，并编写单元测试 (第1周目标)
3. **数据分析师工具** (AI概念引入) - 使用pandas处理CSV文件，进行简单数据分析和可视化，包含类型提示 (第2周目标)
4. **文本分析器** (AI概念引入) - 读取文本文件，进行简单文本分析（如词频统计、情感分析），使用类型提示和错误处理 (第2周目标)
5. **简单预测模型** (AI概念引入) - 使用pandas和numpy进行基础数据预测，体验数据分析流程 (第3周目标)
